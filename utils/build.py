...
class Setup:
    ...
    def get_setups(self, update) -> None:
        ...
        match[:] = (
            i
            for i in match
            if "Player Count" not in i and "href" in i and "docs" not in i
        )
        for i in match:
            href, title = [
                x.group() for x in re.finditer(r"(?<==)('|\").*?\1(?=.*?>)", i)
            ]
            href, title = href[1:-1], title[1:-1]
            title = (
                title.replace("&#39;", "'").replace("&amp;", "&").replace("o&#39;", "'")
            )
            count = re.search(r"<td>(\d+)", i).group(1)
            self.setups[title.lower()] = [f"https://mafiagg.fandom.com/{href}", count]
    ...
    def get_setup_code(self) -> None:
        for setup in self.setups:
            ...
            code = re.search(r'<div class="pi-data-value pi-font">(.*?)</div>', data)
            if code:
