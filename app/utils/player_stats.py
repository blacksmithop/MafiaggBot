...
def get_player_data(id: int=None, username: str= None, report=False):
    if id:
        ...
        # player = PlayerStats(id=data.id, username=data.username, joinedDate=data.createdAt)
        return player
    if username:
        player = GetUser().get_user(id=533610) # TODO: Look up DB for game reports
        if report:
            player = PlayerStats(id=player.id, username=player.username, joinedDate=player.createdAt)
        return player
